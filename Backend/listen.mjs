 ...
 *
 * WHY THIS IS A MODULE (Stage 4). Every PFO server called `server.listen(...)`
 ...
 * `supervisor.log` when PFO did not come back after a reboot — repeated every
 * few seconds by the restart backoff — and it does not once say the thing that
 * would actually help: another copy of PFO is already running.
 *
...
export function listenOrExplain(server, port, host, name, onListening) {
  server.on("error", (error) => {
    if (error.code === "EADDRINUSE") {
      console.error(
        `\n  The PFO ${name} cannot start: port ${port} on ${host} is already in use.\n\n` +
          `  Almost always this means another copy of PFO is already running — a\n` +
          ...
          `  To clear PFO's own leftovers: node Backend/free-dev-ports.mjs\n`,
      );
      process.exit(1);
    }
    if (error.code === "EACCES") {
      console.error(
        `\n  The PFO ${name} cannot start: permission denied binding port ${port}.\n` +
          `  Ports below 1024 need an elevated process on Windows. Either run as\n` +
          `  administrator or pick a port above 1024.\n`,
      );
      process.exit(1);
    }
    console.error(`\n  The PFO ${name} failed to start: ${error.message}\n`);
    process.exit(1);
  });
