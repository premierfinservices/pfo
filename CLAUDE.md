...
- **Unfold Media Corp PC also does development work now** (it is the only
  ...
  - Dev `.env` (or `.env.local`) MUST use `AOS_WEB_HOST=127.0.0.1`
  ...
  - Dev processes must never read or write `C:\AOS\Data` — point
    `AOS_STORAGE_ROOT` at a disposable local folder.
  ...
  - **`npm run dev`'s `predev` hook (`Backend/free-dev-ports.mjs`) kills any
    `node.exe` process listening on `AOS_VITE_PORT`/`AOS_API_PORT`/
    `AOS_STORAGE_PORT`/`AOS_MAIL_PORT`, which default to 5173/4321/4319/4320
    ...
    here, e.g.:
    ```powershell
    $env:AOS_VITE_PORT=5273; $env:AOS_API_PORT=4421
    $env:AOS_STORAGE_PORT=4419; $env:AOS_MAIL_PORT=4420
    $env:AOS_WEB_HOST="127.0.0.1"; $env:AOS_DB_NAME="aos_dev"
    $env:AOS_STORAGE_ROOT="C:\AOS\Dev\Data"
    npm run dev
    ```
