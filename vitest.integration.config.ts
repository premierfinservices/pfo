 ...
 *
 * `PFO_DB_NAME` is set here rather than in `.env` so the suite physically
 * cannot address the office database: `loadDotEnv` never overwrites a variable
 * that is already set.
 *
 * `PFO_DB_USER`/`PFO_DB_PASSWORD` are resolved here for the same reason, not
 ...
 * `postgres`, false the moment an office switches `PFO_DB_USER` to `aos_app`
 ...
 * `PFO_DB_ADMIN_USER`/`_PASSWORD` if the machine's `.env` set them, else fall
 * back to `PFO_DB_USER`/`_PASSWORD` (a plain dev machine, still `postgres`).
 */
loadDotEnv();
const testDbUser = process.env.PFO_DB_ADMIN_USER?.trim() || process.env.PFO_DB_USER || "postgres";
const testDbPassword = process.env.PFO_DB_ADMIN_USER?.trim()
...
export default defineConfig({
  test: {
    ...
    env: {
      ...
      // is exactly the class of mistake `PFO_REQUIRE_DB_NAME` (db.ts) exists
      ...
      // directory.
      PFO_STORAGE_SERVER_URL: "http://127.0.0.1:4329",
      ...
      PFO_MAIL_SERVER_URL: "http://127.0.0.1:4330",
    },
